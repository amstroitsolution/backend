...
exports.getAll = async (req, res) => {
  try {
    const items = await TrendingItem.find({ isActive: true }).sort({ order: 1, createdAt: -1 });
    res.json(items);
  } catch (err) {
...
exports.create = async (req, res) => {
  try {
    ...
    const item = new TrendingItem({ ...req.body, images });
    await item.save();

    // Auto-create inquiry for new trending item
    await createAutoInquiry(
      item._id,
      'TrendingItem',
      `New Trending Item: ${item.title}`,
      `Auto-generated inquiry for new trending item "${item.title}". Price: ₹${item.price || 'Not specified'}. Created on ${new Date().toLocaleDateString()}. This trending item may need review or promotional setup.`
    );
...
exports.update = async (req, res) => {
  try {
    ...
    const item = await TrendingItem.findByIdAndUpdate(
      req.params.id,
      { ...req.body, images },
      { new: true }
    );
